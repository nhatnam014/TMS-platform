...
The system SHALL provide `PATCH /api/v1/trip-costs/:id` accepting `{ name?: string, isActive?: boolean, amount?: number }`. Partial updates are supported. `amount` may be set to `null` to remove the default price.
...
- **THEN** the response is HTTP 200 with an array of `{ id, name, isActive, amount }` objects, where `amount` is a number or `null`
