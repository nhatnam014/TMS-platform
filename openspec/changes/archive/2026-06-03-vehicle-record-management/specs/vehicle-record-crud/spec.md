...
- Tên tài xế (text, optional)
- SĐT (text, optional)
- Loại xe (text, optional)
- Biển số (text, optional)
- Hạn đăng kiểm xe (date, optional)
- Hạn bảo hiểm xe (date, optional)
- Hạn cà vẹt xe (date, optional)
- Ghi chú (textarea, optional)
- Moocs section: dynamic list, each row has — Số mooc (text), Hạn ĐK (date, optional), Hạn BH (date, optional), Hạn CV (date, optional) — plus a remove button
- "+ Thêm mooc" button to add a blank mooc row

#### Scenario: Add mooc row

- **WHEN** user clicks "+ Thêm mooc"
- **THEN** a new blank mooc row appears in the form with fields for soMooc, hanDangKiem, hanBaoHiem, hanCaVet

#### Scenario: Remove mooc row

- **WHEN** user clicks the remove button on a mooc row
