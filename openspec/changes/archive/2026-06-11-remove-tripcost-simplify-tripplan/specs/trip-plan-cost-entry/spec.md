...
**Reason**: The `tripCostId` FK field is being removed along with the `TripCost` catalog table. The `costName` snapshot column on `TripPlanCost` remains but is now the primary identifier (no catalog to look up). The "snapshot survives catalog deletion" concern is no longer applicable since there is no catalog.
...
**Reason**: The dropdown was sourced from the TripCost catalog, which is removed. The UI now uses plain text/amount inputs.
**Migration**: See `trip-cost-freeform-entry` spec.

### Requirement: Trip plan response includes cost breakdown

**Reason**: With `tripCostId` removed from `TripPlanCost`, the `costs` array shape changes. `tripCostId` field is dropped from each cost item. See updated `trip-plan-cost-form` spec for the new response shape.
