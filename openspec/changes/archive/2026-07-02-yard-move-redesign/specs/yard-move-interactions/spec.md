...
**Reason**: No status lifecycle remains on `YardMove` — the entity is a flat trip record with no workflow.
**Migration**: Remove the inline status-transition buttons ("Bắt đầu", "Hoàn thành", "Hủy") from the Actions column.

### Requirement: Add cost to yard move

**Reason**: Cost tracking on yard moves is removed entirely.
**Migration**: Remove the "+ Chi phí" button and the `CostModal` component from the yard moves page.
