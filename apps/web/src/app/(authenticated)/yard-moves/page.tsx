...
interface YardMoveRow {
  ...
  toZone: FactoryZone;
  notes: string | null;
  container: { id: string; containerNumber: string; sizeType: string; status: string } | null;
  location: { id: string; code: string; name: string } | null;
  costs: YardMoveCost[];
}

interface ContainerOption { id: string; containerNumber: string; sizeType: string; }
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#fff", borderRadius: 12, padding: 28, width: 500, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 8px 40px rgba(0,0,0,0.18)" }}>
...
function CreateYardMoveModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/containers?limit=200").then((r) => r.json()),
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!date || !containerId || !fromZone || !toZone || !locationId) {
    ...
    try {
      const res = await fetch("/api/yard-moves", {
        ...
        body: JSON.stringify({ date, containerId, fromZone, toZone, locationId, notes: notes || undefined }),
      });
      if (!res.ok) {
      ...
      {loadingOptions ? (
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            ...
            <select value={containerId} onChange={(e) => setContainerId(e.target.value)} style={inputStyle} required>
              <option value="">-- Chọn container --</option>
              {containers.map((c) => (
                <option key={c.id} value={c.id}>{c.containerNumber} ({c.sizeType})</option>
              ))}
            </select>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              ...
              <select value={fromZone} onChange={(e) => setFromZone(e.target.value as FactoryZone)} style={inputStyle} required>
                <option value="">-- Chọn --</option>
                {FACTORY_ZONES.map((z) => <option key={z} value={z}>{ZONE_LABELS[z]}</option>)}
              </select>
            </div>
            <div>
              <label style={labelStyle}>Đến khu vực *</label>
              <select value={toZone} onChange={(e) => setToZone(e.target.value as FactoryZone)} style={inputStyle} required>
                <option value="">-- Chọn --</option>
                {FACTORY_ZONES.map((z) => <option key={z} value={z}>{ZONE_LABELS[z]}</option>)}
              </select>
            </div>
          ...
          <button type="submit" disabled={submitting} style={{ padding: "9px 18px", background: "#3b82f6", color: "#fff", border: "none", borderRadius: 7, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
...
function CostModal({ moveId, onClose, onAdded }: { moveId: string; onClose: () => void; onAdded: () => void }) {
    ...
    <Modal title="Thêm chi phí lệnh bãi" onClose={onClose}>
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <div>
          ...
          <select value={type} onChange={(e) => setType(e.target.value as YardCostType)} style={inputStyle}>
        ...
        <div>
          ...
          <input type="number" min={1} value={amount} onChange={(e) => setAmount(e.target.value)} style={inputStyle} placeholder="500000" required />
        </div>
        <div>
          <label style={labelStyle}>Ghi chú</label>
        ...
        {formError && <p style={{ color: "#dc2626", fontSize: 13 }}>{formError}</p>}
        <button type="submit" disabled={submitting} style={{ padding: "9px 18px", background: "#10b981", color: "#fff", border: "none", borderRadius: 7, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
...
export default function YardMovesPage() {
  ...
  const btnBase: React.CSSProperties = { padding: "4px 10px", border: "none", borderRadius: 5, fontSize: 12, fontWeight: 600, cursor: "pointer" };
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button
            ...
            style={{ padding: "7px 16px", background: "#3b82f6", color: "#fff", border: "none", borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: "pointer" }}
          >
            + Tạo lệnh
          </button>
        </div>
      ...
      {actionError && (
        <p style={{ color: "#dc2626", background: "#fef2f2", padding: "10px 14px", borderRadius: 8, marginBottom: 16, fontSize: 13 }}>{actionError}</p>
      )}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0" }}>
              {["Ngày", "Container", "Từ khu vực", "Đến khu vực", "Địa điểm", "Trạng thái", "Thao tác"].map((h) => (
                ...
                return (
                  <tr key={m.id} style={{ borderBottom: "1px solid #f1f5f9", background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                    ...
                    <td style={{ padding: "10px 14px", fontSize: 13 }}>
                      <span style={{ fontWeight: 600, fontFamily: "monospace" }}>{m.container?.containerNumber ?? "—"}</span>
                      {m.container && <span style={{ fontSize: 11, color: "#94a3b8", marginLeft: 4 }}>({m.container.sizeType})</span>}
                    </td>
                    <td style={{ padding: "10px 14px", fontSize: 12, color: "#64748b" }}>{ZONE_LABELS[m.fromZone]}</td>
                    <td style={{ padding: "10px 14px", fontSize: 12, color: "#64748b" }}>{ZONE_LABELS[m.toZone]}</td>
                    <td style={{ padding: "10px 14px", fontSize: 13 }}>{m.location?.name ?? "—"}</td>
      ...
      {showCreate && (
        <CreateYardMoveModal
          ...
          onCreated={() => { setShowCreate(false); fetchMoves(); }}
        />
      )}
      {costMoveId && (
        <CostModal
          ...
          onAdded={() => { setCostMoveId(null); fetchMoves(); }}
        />
      )}
    </div>
  );
