...
function UploadSection({ title, endpoint, description }: UploadSectionProps) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      if (!res.ok) { setError(data.message ?? `Lỗi ${res.status}`); return; }
    ...
    <div style={{ background: "#fff", borderRadius: 10, boxShadow: "0 1px 4px rgba(0,0,0,0.07)", padding: 24, marginBottom: 24 }}>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        <input
          ...
          type="file"
          accept=".xlsx,.xls"
          style={{ fontSize: 13, border: "1px solid #d1d5db", borderRadius: 6, padding: "6px 10px" }}
        />
        <button
          onClick={handleUpload}
          disabled={uploading}
          style={{ padding: "8px 18px", background: "#3b82f6", color: "#fff", border: "none", borderRadius: 6, fontSize: 14, cursor: uploading ? "not-allowed" : "pointer", opacity: uploading ? 0.7 : 1, fontWeight: 500 }}
        >
      ...
      {error && (
        <div style={{ marginTop: 14, padding: "10px 14px", background: "#fef2f2", borderRadius: 6, color: "#dc2626", fontSize: 13 }}>
          {error}
        </div>
      )}
      ...
      {result && (
        <div style={{ marginTop: 14 }}>
          <div style={{ padding: "10px 14px", background: "#f0fdf4", borderRadius: 6, color: "#166534", fontSize: 13, fontWeight: 600, marginBottom: result.warnings.length || result.errors.length ? 10 : 0 }}>
            Nhập thành công {result.imported} bản ghi
          </div>
          {result.warnings.length > 0 && (
            <details style={{ marginBottom: 8 }}>
              <summary style={{ fontSize: 13, color: "#92400e", cursor: "pointer", padding: "8px 12px", background: "#fffbeb", borderRadius: 6 }}>
                {result.warnings.length} cảnh báo (bấm để xem)
              </summary>
              <ul style={{ margin: "6px 0 0 0", paddingLeft: 20 }}>
                {result.warnings.map((w, i) => (
                  <li key={i} style={{ fontSize: 12, color: "#78350f", marginBottom: 3 }}>{w}</li>
                ))}
              </ul>
            </details>
          )}
          {result.errors.length > 0 && (
            <details>
              <summary style={{ fontSize: 13, color: "#991b1b", cursor: "pointer", padding: "8px 12px", background: "#fef2f2", borderRadius: 6 }}>
                {result.errors.length} lỗi bỏ qua (bấm để xem)
              </summary>
              <ul style={{ margin: "6px 0 0 0", paddingLeft: 20 }}>
                {result.errors.map((e, i) => (
                  <li key={i} style={{ fontSize: 12, color: "#7f1d1d", marginBottom: 3 }}>{e}</li>
                ))}
              </ul>
            </details>
          )}
  ...
  return (
    <div style={{ display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
      ...
      <button
        ...
        style={{ padding: "8px 18px", background: "#059669", color: "#fff", border: "none", borderRadius: 6, fontSize: 14, cursor: downloading ? "not-allowed" : "pointer", opacity: downloading ? 0.7 : 1, fontWeight: 500 }}
      >
...
export default function ImportExportPage() {
  ...
  if (role && role !== "ADMIN") {
    return <div style={{ padding: 32, color: "#dc2626" }}>Chỉ ADMIN mới có quyền truy cập trang này.</div>;
  }
  ...
  return (
    <div>
      ...
      {/* Import section */}
      <h2 style={{ fontSize: 15, fontWeight: 600, color: "#374151", marginBottom: 14, borderBottom: "1px solid #e2e8f0", paddingBottom: 8 }}>
      ...
      <UploadSection
        ...
        description="Tải lên sheet 'quản lý xe' — sẽ cập nhật (upsert) xe, tài xế và rơ moóc. Không xóa dữ liệu cũ."
      />
      ...
      {/* Export section */}
      <h2 style={{ fontSize: 15, fontWeight: 600, color: "#374151", marginBottom: 14, borderBottom: "1px solid #e2e8f0", paddingBottom: 8, marginTop: 8 }}>
