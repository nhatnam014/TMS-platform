...
export default function UsersPage() {
  ...
  async function handleCreate() {
    ...
    if (!res.ok) {
      ...
      const msg = body.message ?? "Lỗi tạo người dùng";
      setCreateError(msg);
      toast.error(msg);
      return;
    }
  ...
  async function handleEdit() {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      const msg = body.message ?? "Lỗi cập nhật người dùng";
      setEditError(msg);
      toast.error(msg);
      return;
    }
  ...
  async function handleReset() {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      const msg = body.message ?? "Lỗi đặt lại mật khẩu";
      setResetError(msg);
      toast.error(msg);
      return;
    }
                  ...
                  <td style={{ padding: "10px 16px" }}>
                    <span
                      style={{
                        ...
                        fontSize: 12,
                        fontWeight: 500,
                        background: u.isActive ? "#ecfdf5" : "#f8fafc",
                        color: u.isActive ? "#065f46" : "#94a3b8",
                      }}
                    >
      ...
      {showCreate && (
        ...
        >
          {FIELD(
            "Tên đăng nhập *",
            createForm.username,
            (v) => setCreateForm((f) => ({ ...f, username: v })),
            true,
          )}
          {FIELD(
            "Mật khẩu *",
            createForm.password,
            (v) => setCreateForm((f) => ({ ...f, password: v })),
            true,
            "password",
          )}
          <div style={{ marginBottom: 14 }}>
        ...
        >
          <div style={{ background: "#fff", borderRadius: 10, padding: 28, width: 400 }}>
            ...
            <p style={{ fontSize: 13, color: "#64748b", marginBottom: 16 }}>
              Người dùng:{" "}
              <strong style={{ fontFamily: "monospace" }}>{resetTarget.username}</strong>
            </p>
            {resetError && (
              <p
                style={{
