...
const PAGE_SIZE_CS = 10;

export default function ContainerSizesPage() {
  const toast = useToast();
  const [rows, setRows] = useState<ContainerSizeRow[]>([]);
  ...
  const [editTarget, setEditTarget] = useState<ContainerSizeRow | null>(null);
  ...
  useEffect(() => {
    ...
    params.set("limit", String(PAGE_SIZE_CS));
    if (search.trim()) params.set("search", search.trim());
    if (isActiveFilter) params.set("isActive", isActiveFilter);
    fetch(`/api/container-sizes?${params.toString()}`)
  ...
  async function handleCreate() {
    setCreateError(null);
    const res = await fetch("/api/container-sizes", {
      method: "POST",
    ...
    setCreateForm(EMPTY);
    setRefresh((r) => r + 1);
    toast.success("Thêm size cont thành công");
  }

  async function handleEdit() {
    if (!editTarget) return;
    setEditError(null);
    const res = await fetch(`/api/container-sizes/${editTarget.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    setEditTarget(null);
    setRefresh((r) => r + 1);
    toast.success("Cập nhật size cont thành công");
  }

  async function handleDelete(id: string) {
    setDeleting(id);
    setConfirmDeleteId(null);
    const res = await fetch(`/api/container-sizes/${id}`, { method: "DELETE" });
    setDeleting(null);
    if (res.ok) {
      toast.success("Đã xoá size cont");
      setRefresh((r) => r + 1);
    } else {
      const body = await res.json().catch(() => ({}));
      toast.error(body.message ?? "Lỗi xoá size cont");
    }
  }

  async function handleBulkDelete() {
    const ids = Array.from(selection.selected);
    const res = await fetch("/api/container-sizes/bulk-delete", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (result.skipped.length === 0) {
      toast.success(`Đã xoá ${result.deleted.length} size cont`);
    } else {
  ...
  return (
    <div>
      ...
      >
        <h1 style={{ fontSize: 20, fontWeight: 700 }}>Size cont</h1>
        <button
          onClick={() => {
            setCreateForm(EMPTY);
            setCreateError(null);
        ...
        >
              ...
              <tr style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0" }}>
                <th style={{ padding: "10px 14px", width: 32 }}>
                  <SelectionCheckbox
                    ...
                    ariaLabel="Chọn tất cả size cont"
                  />
                </th>
                {["Mã", "Tên", "Trạng thái", ""].map((h) => (
                  <th
                    key={h}
                    style={{
                      padding: "10px 14px",
                ...
                <tr key={r.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                  <td style={{ padding: "10px 14px" }}>
                    <SelectionCheckbox
                      ...
                      ariaLabel={`Chọn size cont ${r.code}`}
                    />
                  </td>
                  ...
                  <td style={{ padding: "10px 14px" }}>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button
                        onClick={() => {
                          setEditTarget(r);
                          setEditForm({ code: r.code, name: r.name });
                          setEditError(null);
                        }}
                        style={{
                          padding: "5px 12px",
          ...
          >
            <span style={{ color: "#94a3b8", fontSize: 13 }}>
              {total === 0
                ? "0 size cont"
                : `Hiển thị ${(page - 1) * PAGE_SIZE_CS + 1}–${Math.min(page * PAGE_SIZE_CS, total)} / ${total}`}
            </span>
            {totalPages > 1 && (
        ...
        >
          <div style={{ background: "#fff", borderRadius: 10, padding: 28, width: 360 }}>
            ...
            <p style={{ fontSize: 14, color: "#374151", marginBottom: 20 }}>
              Bạn có chắc muốn xoá size cont này? Nếu đang được sử dụng trong kế hoạch chuyến, thao
              tác sẽ thất bại.
            </p>
      ...
      {showBulkConfirm && (
        <ConfirmDialog
          ...
          message={`Bạn có chắc muốn xoá vĩnh viễn ${selection.selectedCount} size cont đã chọn? Những mục đang được sử dụng trong kế hoạch chuyến sẽ được bỏ qua.`}
          danger
